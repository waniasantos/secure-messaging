...
class SecureMessagingServer:
    ...
    def __init__(self):
        self.sessions = {}  # {client_id: {websocket, key_c2s, key_s2c, seq_recv, seq_send, salt}}
    ...
    async def perform_handshake(self, websocket):
        try:
            # WebSocket: A primeira mensagem é o Client Hello
            ...
            try:
                ...
                print(f"[REJEITADO] {e}")
                # Enviar erro e fechar
                # (Opcional: enviar msg de erro formatada)
                return None
            ...
            # Transcript: pk_S || client_id || pk_C || salt
    ...
    async def process_messages(self, websocket, client_id):
        ...
        try:
            async for frame in websocket:
                # WebSocket garante que recebemos o frame inteiro
                
                # Nonce esperado (12B) + Sender (16B) + Recip (16B) + Seq (8B) + Cipher
    ...
    async def forward_message(self, recipient_id, sender_id, message):
        ...
        try:
            ...
            # Se falhar o envio (cliente desconectado), ignoramos ou logamos
            pass
    ...
    async def start(self, host='127.0.0.1', port=8888):
        ...
        async with websockets.serve(self.handle_client, host, port):
            await self.stop_future  # Aguarda sinal de parada
